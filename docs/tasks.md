...
+ После подключении пользователю видно 10 последних сообщений. 
...
#### Сборка (3 балла)
Упаковать какой-либо написаный ранее Web-сервис в контейнер (Docker). При запуске контейнера сервис должен автоматически подниматься и слушать порт 80.

#### Сторонний сервис (2-5 баллов)
...
2 балла начисляются за один контейнер, ещё плюс 3 балла если сервис состоит из нескольких контейнеров (рекомендуется использовать docker-compose).
